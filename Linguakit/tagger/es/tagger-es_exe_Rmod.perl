...
sub classif {
	...
	foreach my $cat (@{$C}) {
		...
		$found{$cat}=0;
		### desambiguar primeiro os features da direita (o_PP/DA alimento_NC/VMI)
		my %R_amb=();#<hash><integer>
		my %R_noamb=();#<hash><integer>
		foreach my $feat_restr (@{$F}) {
		    if  ($feat_restr =~ /_R_/) {
			($feat) = $feat_restr =~ /^[a-z]+\_[0-9]+\_[0-9]\_([R]\_[^ \_]+)/;
			$R_amb{$feat} = $PriorProb{$cat}{$feat};
		    }
		}
		my $top=0;#<integer>
		foreach my $f (sort {$R_amb{$b} <=> $R_amb{$a} }  keys %R_amb ) {
		    if (!$top && $f) {
			$R_noamb{$f}++;
			#print STDERR "--#$f# - 	#$R_amb{$f}#\n";
			$top=1;
		    }
		...
		foreach my $feat_restr (@{$F}) {
			...
			if ($feat =~ /^R_/) {
			    my ($feat_tmp) = $feat =~ /(R_[^_]+)/;
			    if (!$R_noamb{$feat_tmp}) {
				#print STDERR "--#$feat_tmp#\n";
				next
			    }
			}
